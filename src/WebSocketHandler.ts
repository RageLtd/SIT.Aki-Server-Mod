...
export class WebSocketHandler {
  ...
  constructor(webSocketPort: number, logger: ILogger) {
    ...
    webSocketServer.addListener("listening", () => {
      ...
      console.log(
        `COOP MOD: Web Socket Server is listening on ${webSocketPort}`
      );
      console.log(
        `A temporary Web Socket Server until SPT-Aki open theirs up for modding!`
      );
      console.log(
        `=======================================================================`
      );
  ...
  public areThereAnyWebSocketsOpen(sessions: string[]): boolean {
    for (let session of sessions) {
      if (this.webSockets[session] !== undefined) {
        if (this.webSockets[session].readyState === WebSocket.OPEN) return true;
      }
    }
