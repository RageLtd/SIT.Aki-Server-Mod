...
export class StayInTarkovMod implements IPreAkiLoadMod, IPostDBLoadMod {
  ...
  private InitializeVariables(container: tsyringe.DependencyContainer): void {
    ...
    this.httpBufferHandler =
      container.resolve<HttpBufferHandler>("HttpBufferHandler");
      ...
      [
        {
          ...
          action: (url, info: any, sessionId: string, output) => {
            ...
            for (let itemKey in CoopMatch.CoopMatches) {
              ...
              if (m.ConnectedPlayers.length === 0) continue;
            ...
            console.log(`COOP Matches: ${output}`);
            return output;
          },
        },
        {
          ...
          action: (url, info: any, sessionId, output) => {
            ...
            CoopMatch.CoopMatches[info.serverId].Location =
              info.settings.location;
            CoopMatch.CoopMatches[info.serverId].Time =
              info.settings.timeVariant;
            CoopMatch.CoopMatches[info.serverId].WeatherSettings =
              info.settings.timeAndWeatherSettings;
            CoopMatch.CoopMatches[info.serverId].ExpectedNumberOfPlayers =
              info.expectedNumberOfPlayers;
            ...
            CoopMatch.CoopMatches[info.serverId].Password =
              info.password !== undefined ? info.password : undefined;
        ...
        result.getLocation = (
          ...
          if (this.locationData[info.locationId] === undefined) {
            ...
            this.locationData[info.locationId].Loot =
              this.locationData[info.locationId].Data.Loot;
          }
  ...
  postDBLoad(container: tsyringe.DependencyContainer): void {
    ...
    const locations = StayInTarkovMod.container
